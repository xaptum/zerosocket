...
int test_compose() {
	...
	jsmnval_t val = {.str = "test1"};
	jsmn_compose(&composer,"test", val, JSMN_STRING);

	jsmnval_t val2 = {.str = "mouse"};
	jsmn_compose(&composer,"cat", val2, JSMN_STRING);

	jsmnval_t val3 = {.str = "hyde"};
	jsmn_compose(&composer,"Jekyl", val3, JSMN_STRING);

	jsmnval_t val4 = {.str = ""};
	jsmn_compose(&composer,"auth", val4, JSMN_OPENOBJ);

	jsmnval_t auth1 = {.str = "username"};
	jsmn_compose(&composer,"ZUSR", auth1, JSMN_STRING);

	jsmnval_t auth2 = {.str = "password"};
	jsmn_compose(&composer,"ZPAS", auth2, JSMN_STRING);

	jsmn_close(&composer,1);

	jsmnval_t val5 = {.str = "farse"};
	jsmn_compose(&composer,"phart", val5, JSMN_STRING);

	jsmn_close(&composer,JSMN_MAX_LEVEL);

	printf("[composition]>>>%s<<<\n",composer.buff);
	printf("[safe_escape]>>>%s<<<\n",jsmn_safe_escape(&composer));
	return 0;
}

#ifdef NON_STRICT_TEST
...
/*	test(test_empty, "general test for a empty JSON objects/arrays");
	...
	test(test_compose,"test composing of JSON object");
